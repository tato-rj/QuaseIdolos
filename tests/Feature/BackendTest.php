...
{
    /** @test */
    public function a_gig_is_automatically_closed_at_a_specified_time_confirming_any_unfinished_song_requests()
    {
        ...
        $this->assertEquals(SongRequest::completed()->count(), 1);

        $this->artisan('gigs:close')->assertSuccessful();

        $this->assertEquals(SongRequest::count(), 3);

        $this->assertEquals(SongRequest::completed()->count(), 2);
    }
}
