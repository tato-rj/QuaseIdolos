	...
	<div class="{{$sizes ?? 'col-lg-5 col-md-8 col-12'}} mx-auto d-flex">
		<form onSubmit="return false;" class="form-borderless mb-4 mx-0 w-100">
				<div class="position-relative w-100 align-items-center">
					<input data-url="{{$url}}" data-target="#{{$target ?? null}}" data-paginate="{{$paginate ?? null}}" data-table="{{$table ?? null}}" value="{{request()->input}}" type="text" name="{{$name ?? 'search'}}" autocomplete="off" placeholder="{{$placeholder}}" class="form-control">
